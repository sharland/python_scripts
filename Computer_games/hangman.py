...
def displayBoard(HANGMANPICS, missedLetters,
    ...
    for letter in blanks: #show the secret word with spaces in between each letter
        print(letter,end=" ")
    print()
def getGuess(alreadyGuessed): #returns the letter the player entered.  This function makes sure the player enters a single letter and not something else
    while True:
...
while True:
    ...
    guess = getGuess(missedLetters + correctLetters) #let the player type in a latter

    if guess in secretWord:
        correctLetters = correctLetters + guess

        foundAllLetters = True #check that player has won
    ...
    if len(missedLetters) == len(HANGMANPICS) - 1:
        displayBoard(HANGMANPICS, missedLetters,coreectLetters,secretWord)
